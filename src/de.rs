...
struct PreservedValueAccess {
    val: Option<JsValue>,
}

impl PreservedValueAccess {
    fn new(val: JsValue) -> Self {
        Self { val: Some(val) }
    }
}

impl<'de> de::MapAccess<'de> for PreservedValueAccess {
    type Error = Error;
    ...
    fn next_value_seed<V: de::DeserializeSeed<'de>>(&mut self, seed: V) -> Result<V::Value> {
        let de = Deserializer {
            value: JsValue::from_f64(self.val.take().unwrap_throw().into_abi() as f64),
        };
        seed.deserialize(de)
    }
}
...
impl<'de> de::Deserializer<'de> for Deserializer {
    ...
    fn deserialize_struct<V: de::Visitor<'de>>(
        ...
        if name == crate::preserve::PRESERVED_VALUE_MAGIC {
            // How do we get a V::Value here? If this visitor is autogenerated, I think we *have* to call
            // visitor.visit_map. That's ok, we want to actually return a PreservedValueDeWrapper...
            visitor.visit_map(PreservedValueAccess::new(self.value))
        } else {
